...
import deprecated.game.playeraction.BlockEffect;
import deprecated.game.playeraction.BoostEffect;
import deprecated.game.playeraction.DamageEffect;
import deprecated.game.playeraction.DropEffect;
import deprecated.game.playeraction.HealingEffect;
import deprecated.game.playeraction.ItemEffect;
import deprecated.game.playeraction.RecoilEffect;
import deprecated.game.playeraction.SwitchEffect;
import deprecated.game.playeraction.AttackEffect.EffectType;
import deprecated.game.playeraction.BlockEffect.BlockType;
...
public class GenericAttackBuilder implements AttackBuilder {
	...
	private void initializeKeyWords() {
		...
		attackKeyWords.put("dragged", EffectType.SWITCH);
	}
	...
	private DamageEffect buildDamageEffect(String line) {
		...
		if(line.contains("It doesn't affect")){
			...
			effect.setEffectiveness(GameUtil.Effectiveness.IMMUNE);
			return effect;
		}
		
		effect.setCrit(line.contains("A critical hit!"));
		if(line.contains("It's not very effective...")){
			effect.setEffectiveness(GameUtil.Effectiveness.NOT_VERY_EFFECTIVE);
		}
		else if(line.contains("It's super effective!")){
			effect.setEffectiveness(GameUtil.Effectiveness.SUPER_EFFECTIVE);
		}
		else{
			effect.setEffectiveness(GameUtil.Effectiveness.NEUTRAL);
		}
	...
	private AttackEffect buildSwitchEffect(String line) {
		SwitchEffect effect = new SwitchEffect();
		StringTokenizer st = new StringTokenizer(line);
		String pokemon = "";
		String next;
		while(!(next = st.nextToken()).equals("was")){
			pokemon = next;
		}
		effect.setPokemon(pokemon);
		return effect;
	}
